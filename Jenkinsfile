pipeline {
    ...
    stages {
        ...
        stage('Build Docker Image') {
            steps { sh """ docker rmi -f ${IMAGE_NAME}:${IMAGE_TAG} || true docker build -t ${IMAGE_NAME}:${IMAGE_TAG} . """ } } 
        ...
        stage('Deploy to Nexus') { steps { withMaven( globalMavenSettingsConfig: 'settings.xml',  // Jenkins managed settings.xml maven: 'manen3',                           // Must match Global Tool name in Jenkins traceability: true ) { sh 'mvn deploy' } } }
    }
}
